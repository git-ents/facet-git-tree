...
. `Array` and `Vec` — sequence elements are recorded as entries named by their zero-based index, encoded as a link:#serialization.design.trees.ordering[zero-padded ordinal name].
. `Map` — each entry is named by the textual form of its key and resolves to the encoded value.
